...
otherwise 
   ...
   if iscell(breaks) 
      ...
      x = [breaks(2:l) linspace(breaks(1),breaks(l+1),step_npoints)]; 
      v = ppual(f,x);  
...
if nargout==0 
   if iscell(x) 
      ...
      case 2 
         v = squeeze(v); roughp = 1+(step_npoints-1)/5; 
         vv = reshape(cat(1,... 
              permute(v(:,1:5:step_npoints,:),[3,2,1]),... 
              repmat(NaN,[1,roughp,2]),... 
              permute(v(:,:,1:5:step_npoints),[2,3,1]),... 
              repmat(NaN,[1,roughp,2])), ... 
                    [2*roughp*(step_npoints+1),2]); 
         plot(vv(:,1),vv(:,2)) 
      case 3 
